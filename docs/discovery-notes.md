...
- **Status**: open; session-02 implementation owns closure (fold into Deliverable 0 pre-flight commit OR into the FFI module commit since teardown ordering is in scope when the LibRaw teardown gets wired).
...
- **Status**: open (binding trigger fires during session 02 implementation at Deliverable 0).
